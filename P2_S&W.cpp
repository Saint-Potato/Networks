...
class Receiver{
	private:
		...
		int receive(int packet_no){
			if (packet_no < no_messages_received){
				cout << "Receiver - \tDiscarded packet no " << packet_no << endl;
	...
	public:
		...
		void showPacketsReceived(){
			cout << "Receiver - \tPackets received = " << no_messages_received << endl;
		}
		...
		void send(Receiver& r){
			...
			while (cur_packet_sending < noPackets){
				try{
					time_t sent , now;
					time(&sent);
					cout << "Sender - \tSent packet number " << cur_packet_sending << endl;
					int ackNo = -3;
					sleep(1);
					time(&now);
					...
					if (ackNo < 0){
						cout << "Sender - \tResending packet number " << cur_packet_sending << endl;
